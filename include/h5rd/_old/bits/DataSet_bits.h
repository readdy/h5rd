...
inline DataSet::DataSet(handle_id handle, const DataSetType &memoryType, const DataSetType &fileType)
...
inline void VLENDataSet::append(const dimensions &dims, std::vector<T> *const data) {
    {
        ...
        // todo log::trace("appending to vlen data set with data size = ({})", result.str());
    }
    {
        auto fs = getFileSpace();
        if (dims.size() != fs.ndim()) {
            // todo log::error("Tried to append data with ndims={} to set with ndims={}", dims.size(), fs.ndim());
    ...
    std::vector<hvl_t> traj;
    {
        ...
        // todo log::trace("appending n={} vlen entries", n);
        traj.reserve(n);
        for (auto i = 0; i < n; ++i) {
            auto val = &data[i];
            hvl_t entry;
...
template<typename T>
inline void DataSet::append(const dimensions &dims, const T *const data) {
    {
        std::stringstream result;
        std::copy(dims.begin(), dims.end(), std::ostream_iterator<int>(result, ", "));
    ...
    if (H5Dwrite(hid(), memoryType.hid(), memorySpace.hid(), fileSpace.hid(), H5P_DEFAULT, data) < 0) {
        // todo log::error("Error with data set {}", hid());
        H5Eprint(H5Eget_current_stack(), stderr);
    }
}
...
inline VLENDataSet::VLENDataSet(handle_id handle, const DataSetType &memoryType, const DataSetType &fileType)
        : Object(std::make_shared<DataSetHandle>(handle)), memoryType(memoryType), fileType(fileType),
          memorySpace(-1) {
    if (handle < 0) {
        // todo log::critical("tried to create a data set with negative handle!");
    }
}
