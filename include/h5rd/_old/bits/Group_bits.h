...
inline Group::Group(handle_id handle, const std::string &path)
...
inline void Group::write(const std::string &dataSetName, const std::string &string) {
    ...
    if (H5LTmake_dataset_string(**handle, dataSetName.c_str(), string.c_str()) < 0) {
        // todo log::warn("there was a problem with writing {} into a hdf5 file.", string);
    }
}
...
inline group_info Group::info() const {
    group_info info {};
    if (H5Gget_info(hid(), &info) < 0) {
        // todo log::critical("Failure to get group info!");
...
inline std::vector<std::string> Group::sub_elements(H5O_type_t type) const {
    ...
    for (std::size_t i = 0; i < group_info.nlinks; ++i) {
        ...
        if (oinfo.type == type) {
            ...
            H5Lget_name_by_idx(hid(), ".", H5_INDEX_NAME, H5_ITER_INC, i, c_string.data(), (std::size_t) size,
                               H5P_DEFAULT);
...
inline Group Group::subgroup(const std::string &name) {
    ...
    if (gid < 0) {
        // todo log::critical("could not open subgroup {}", name);
        H5Eprint(H5Eget_current_stack(), stderr);
    }
...
template<typename T>
inline void
Group::read(const std::string &ds_name, std::vector<T> &array, DataSetType memoryType, DataSetType fileType) {
...
template<>
inline void
Group::write<short>(const std::string &dataSetName, const dimensions &dims, const short *data) {
    H5LTmake_dataset_short(hid(), dataSetName.data(), static_cast<int>(dims.size()), dims.data(), data);
}

template<>
inline void Group::write<int>(const std::string &dataSetName, const dimensions &dims, const int *data) {
    H5LTmake_dataset_int(hid(), dataSetName.data(), static_cast<int>(dims.size()), dims.data(), data);
}

template<>
inline void Group::write<long>(const std::string &dataSetName, const dimensions &dims, const long *data) {
    H5LTmake_dataset_long(hid(), dataSetName.data(), static_cast<int>(dims.size()), dims.data(), data);
}

template<>
inline void
Group::write<float>(const std::string &dataSetName, const dimensions &dims, const float *data) {
    H5LTmake_dataset_float(hid(), dataSetName.data(), static_cast<int>(dims.size()), dims.data(), data);
}

template<>
inline void
Group::write<double>(const std::string &dataSetName, const dimensions &dims, const double *data) {
